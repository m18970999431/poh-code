...
export class Cloud {
  ...
  initinfo() { // 加载用户Profile、Department、CommonACL数据
    let currentUser = Parse.User.current()
    console.log("Important:",currentUser)
    if (currentUser) {
      return (<any>this.setProfile()).then((data) => {
        if(data === "stop") return;
  ...
  login(user,type?,role?) {
    ...
    return Parse.User.logIn(user.username, user.password).then(data => {
  ...
  loginoauth(type) {
    ...
    switch (type) {
      case "dingtalk":
        ...
        url = "https://oapi.dingtalk.com/connect/oauth2/sns_authorize?appid=" + appid + "&response_type=code&scope=snsapi_login&state=STATE&redirect_uri=" + rurl
      ...
      case "github":
        ...
        url = "https://github.com/login/oauth/authorize?client_id=" + appid + "&redirect_uri=" + rurl
        // url = url + rurl
        // this.events.publish('browser:oauth', url)
        window.location.href = url
        break
  ...
  checkoauth(search) {
    ...
    Parse.Cloud.run('oauth_gettoken', {
      ...
      Parse.User.logInWith('dingtalk', authData).then((user) => {
  ...
  loginWe7(uniacid): Promise<any> {
    ...
    let url = encodeURI('http://' + host + '/app/index.php?i=' + unicid + '&c=auth&a=cloud_auth');
  ...
  setProfile() {
    ...
    if(this.profile&&this.profile.get("user").id == Parse.User.current().id){
    ...
    return this.queryProfile.find().then(data => {
      if (data.length == 0) {
  ...
  setDepartment() {
    ...
    return this.queryDepartment.find().then(data => {
      let DepartList = {}
  ...
  schemasExtend(schemas) {
    for (let key in schemas) {
      ...
      schemas[key].fields["company"] = { type: "Pointer", targetClass: "Company", name: "所属账套", show:false} //所有对象补全company字段
  ...
  constructor(private http: HttpClient) {
    ...
    // 加载系统Schema对象
    this.schemasExtend(TaxonomySchemas)
    this.schemasExtend(CrmSchemas)
    this.schemasExtend(ShopSchemas)
    this.schemasExtend(VolunteerSchemas)
    this.schemasExtend(PipixiaSchemas)
    this.schemasExtend(HrSchemas)
    this.schemasExtend(CommonSchemas)
    this.schemasExtend(SiteSchemas)
    this.schemasExtend(SurveySchemas)
    this.schemasExtend(MessageSchemas)
    this.schemasExtend(DepartmentSchemas)
    this.schemasExtend(ProjectSchemas)
    this.schemasExtend(ActivitySchemas)
    this.schemasExtend(ProductPaperSchemas)
    this.schemasExtend(BannerSchemas)
    this.schemasExtend(SchemaSchemas)
    this.schemasExtend(OrgSchemas)
    this.schemasExtend(XiaofangSchemas)

    // 系统通用Schema
    this.schemasExtend(UserRoleSchemas)
